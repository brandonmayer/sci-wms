...
def ugridGetMap(request, dbobj):
    """
    Ugrid specific getMap
    """
    from .matplotlib_handler import get_lat_lon_subset_idx,\
        get_nv_subset_idx, get_nearest_start_time
...
def cgridGetMap(request, dbobj):
    ...
    def contourf_response(xcrs,
        ...
        # logger.info("Projecting topology from lat/lon to crs.")
        # lonmerc, latmerc = proj(lon_subset.flatten(), lat_subset.flatten())
        # logger.info("Done projecting topology from lat/lon to crs.")
        ...
        ax.set_xlim(crs_xmin, crs_xmax)
        ax.set_ylim(crs_ymin, crs_ymax)
...
#----------------------------------------------------------------
    ...
    # SPATIAL SUBSET
    logger.info('loading all lon/lat')
    ...
    logger.info('computing spatial subset')
    sub_idx = get_lat_lon_subset_idx(lon, lat, lonmin, latmin, lonmax, latmax)
    ...
    logger.info("computing temporal subset")
    # TEMPORAL SUBSET
    time = [0]
    ...
    if len(variables) == 1:
        ...
        logger.info("Projecting to lon/lat to user crs")
        xcrs,ycrs = proj(lon_subset.flatten(), lat_subset.flatten())
        xcrs.reshape(lon_subset.shape)
        ycrs.reshape(lat_subset.shape)
        logger.info("Finished projection to crs")
        ...
        response = contourf_response(xcrs,
                                     ycrs,
                                     data_subset,
                                     xmin,
                                     ymin,
...
def getMap(request, dataset):
    ...
    logger.debug("getMap request for dataset = {0}".format(dataset))

    # direct the service to the dataset
    try:
        dbobj = dbDataset.objects.get(name=dataset)
    except ObjectDoesNotExist:
        logger.error("{0} does not exist in the database.".format(dataset))
        logger.info("ERROR: " + \
                    repr(traceback.format_exception(exc_type, exc_value, exc_traceback)))

    topology_type = dbobj.topology_type
    logger.info("topology_type: " + topology_type)
    
    if topology_type.lower() == "ugrid":
        logger.info("getMap in ugrid")
        response = ugridGetMap(request, dbobj)
        
    elif topology_type.lower() == "cgrid":
        logger.info("getMap in cgrid")
        response = cgridGetMap(request, dbobj)

    else:
        logger.error("Unknown topology type {0}".format(gridtype))
        return blank_canvas(width, height)
